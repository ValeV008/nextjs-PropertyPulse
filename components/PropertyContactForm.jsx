...
const PropertyContactForm = ({property}) => {
    ...
    useEffect(() => {
        if (state.error) // can be returned by addMessage()
            toast.error(state.error);
        if (state.submitted)
            toast.success("Message sent successfully.");
    },[state])

    if (state.submitted){ // 'submitted' is returned by addMessage()
        return (
            <div className="bg-white p-6 rounded-lg shadow-md">
            <h3 className="text-xl font-bold mb-6">Contact Property Manager</h3>
            <p className="text-green-500 mb-4 text-center">"Your message has been sent"</p>
            </div>
        );
    }
